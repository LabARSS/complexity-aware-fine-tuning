...
def single_token_sys_prompt_from_row(row: pd.Series):
    sys_msg = "The following are multiple choice questions. Write down ONLY the NUMBER of the correct answer and nothing else."
    return sys_msg


def single_token_answer_prompt_from_row(row: pd.Series):
    question = cast(str, row["question"])
    options = [f"{label}. {answer}" for label, answer in zip(row["choices"]["label"], row["choices"]["text"])]
