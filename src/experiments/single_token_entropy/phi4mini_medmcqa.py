...
class Phi4MiniConfig(EstimateDatasetConfig):
    in_filename: str = str(Path(__file__).parent.joinpath("../../../data/source/medmcqa/medmcqa_train.jsonl").resolve())
    out_filename: str = str(
        Path(__file__).parent.joinpath("../../../data/out/single_token_entropy/medmcqa_phi4mini.jsonl").resolve()
    )
    dump_every: int = 100
...
estimate_dataset(Phi4MiniConfig())
